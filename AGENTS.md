...
# 特定ファイルだけ apply。brew bundle や apm install を走らせたくない時に使う
...
herdr のユーザー設定は `dot_config/herdr/config.toml`（→ `~/.config/herdr/config.toml`）で管理する。

- **`config.toml` のみ管理対象。** テーマ・UI 設定が入る
- **`session.json` / `*.log` / `*.sock` は管理しない。** ワークスペース状態・ログ・ソケットは herdr が実行時に自動生成するマシン固有物のため。`chezmoi add ~/.config/herdr`（ディレクトリごと追加）は使わず、`config.toml` 単体を追加する
