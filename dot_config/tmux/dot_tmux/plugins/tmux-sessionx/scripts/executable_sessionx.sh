...
get_sorted_sessions() {
	...
	# Debug: print raw sessions when SESSIONX_DEBUG is set
	raw_sessions=$(tmux list-sessions)
	if [[ -n "$SESSIONX_DEBUG" ]]; then
		echo "DEBUG: raw tmux list-sessions:" >&2
		echo "$raw_sessions" >&2
	fi
	sessions=$(echo "$raw_sessions" | sed -E 's/:.*$//' | grep -Fxv "$last_session")
	if [[ -n "$SESSIONX_DEBUG" ]]; then
		echo "DEBUG: filtered sessions (after sed|grep):" >&2
		echo "$sessions" >&2
	fi
...
handle_input() {
	...
	# back binding removed per user request
}
