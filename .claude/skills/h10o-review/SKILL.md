...
**以下の2点を1メッセージでまとめて確認する:**

> **1. レビュー領域**（複数可、all で全領域）
...
> 6. all（全領域）
>
> **2. 対象範囲**（ファイルパス・ディレクトリ・`git diff` の貼り付けなど）

選択された領域の `references/*.md` のみを読み込み、対象外の領域はスキップする。

### 引数ありの場合

ファイル・パス・差分が渡された場合は領域確認をスキップし、全領域でレビューを実行する（Tailwind は対象コードに `className` へのTailwindユーティリティクラス、または `tailwind.config.*` の存在が確認できた場合のみ）。
...
問題の有無にかかわらず最低3観点を確認し、問題が見つからなかった観点は「確認済み」として明示する。
